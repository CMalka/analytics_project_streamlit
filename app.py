...
Expects a `data/` subfolder next to app.py containing one gzipped CSV per
fiscal year: data/fy2014.csv.gz, data/fy2015.csv.gz, data/fy2016.csv.gz,
data/fy2017.csv.gz (add more fy<year>.csv.gz files if you have more years).
Each file must have the original NYC payroll column headers
(Fiscal Year, Agency Name, Last Name, First Name, ... Total Other Pay).

If `data/` isn't found, the app also accepts a single, uncompressed
Citywide_Payroll_Data__Fiscal_Year_.csv in the same folder, or lets you type
a path to either.
"""
...
DATA_DIR = "data"
DATA_GLOB = "fy*.csv.gz"
FALLBACK_CSV = "Citywide_Payroll_Data__Fiscal_Year_.csv"
...
# --------------------------------------------------------------------------
# Data loading & cleaning (cached so the ~2.2M-row dataset is parsed once)
# --------------------------------------------------------------------------
@st.cache_data(show_spinner="Loading and cleaning payroll data (first run only)...")
def load_data(csv_paths: list) -> pd.DataFrame:
    """csv_paths: list of file paths. Each may be a plain .csv or a .csv.gz
    (pandas infers gzip compression automatically from the .gz extension)."""
    dtypes = {
        "Fiscal Year": "int16",
    ...
    for path in csv_paths:
        for chunk in pd.read_csv(path, dtype=dtypes, chunksize=300_000, low_memory=False):
            chunk.columns = (
    ...
    if not chunks:
        raise ValueError("No data rows were loaded from the given file(s).")
...
# --------------------------------------------------------------------------
# Locate the data: data/fy*.csv.gz (preferred) -> single CSV -> ask the user
# --------------------------------------------------------------------------
st.title("\U0001F4CA NYC Citywide Payroll Explorer")

split_files = sorted(glob.glob(os.path.join(DATA_DIR, DATA_GLOB)))

if split_files:
    data_source = split_files
    st.caption(f"Loaded {len(split_files)} file(s) from `{DATA_DIR}/`: "
               + ", ".join(os.path.basename(p) for p in split_files))
elif os.path.exists(FALLBACK_CSV):
    data_source = [FALLBACK_CSV]
else:
    st.warning(
        f"Couldn't find a `{DATA_DIR}/{DATA_GLOB}` set of files or "
        f"`{FALLBACK_CSV}` next to app.py."
    )
    user_path = st.text_input(
        "Path to a CSV file, a .csv.gz file, or a folder containing fy*.csv.gz files",
        value="",
    )
    if not user_path:
        st.stop()
    if not os.path.exists(user_path):
        st.error(f"Path not found: {user_path}")
        st.stop()
    if os.path.isdir(user_path):
        data_source = sorted(glob.glob(os.path.join(user_path, DATA_GLOB)))
        if not data_source:
            st.error(f"No {DATA_GLOB} files found in that folder.")
            st.stop()
    else:
...
if df.empty:
    st.error("The loaded data has 0 rows. Check that the source file(s) aren't empty or corrupted.")
    st.stop()
